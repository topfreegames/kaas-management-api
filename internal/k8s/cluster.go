...
func (k Kubernetes) GetCluster(clusterName string, namespace string) (clusterapiv1beta1.Cluster, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			return clusterapiv1beta1.Cluster{}, util.NewClientError(err, fmt.Sprintf("The requested cluster %s was not found in namespace %s!", clusterName, namespace), http.StatusNotFound)
...
func (k Kubernetes) ListClusters(namespace string) (clusterapiv1beta1.ClusterList, error) {
	...
	if errors.IsNotFound(err) {
		return clusterapiv1beta1.ClusterList{}, util.NewClientError(err, fmt.Sprintf("Could not find any cluster in namespace %s!", namespace), http.StatusNotFound)
	} else if statusError, isStatus := err.(*errors.StatusError); isStatus {
